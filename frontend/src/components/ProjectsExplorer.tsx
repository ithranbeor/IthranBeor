...
/** Play-badge overlay used on any video thumbnail so it's visually distinct from a photo. */
function PlayBadge() {
  return (
...
function ProjectCard({ project, onOpen }: { project: Project; onOpen: () => void }) {
  ...
  const coverIsVideo = cover ? isVideo(cover.src) : false;
          ...
          {coverIsVideo ? (
            <div className="relative">
              <video
                ...
                muted
                playsInline
                className="block h-52 w-full object-cover transition duration-500 group-hover:scale-[1.02]"
              />
              <PlayBadge />
            </div>
          ) : (
...
function ProofStack({
  ...
  const featuredIsVideo = isVideo(featured.src);
  ...
  return (
    <div className="min-w-0 w-full space-y-4">
      ...
      >
        ...
        {featuredIsVideo && <PlayBadge />}
      </motion.button>

      {/* Gallery — horizontally scrollable row, new items append sideways */}
      {rest.length > 0 && (
        <div className="apple-scroll flex w-full min-w-0 gap-4 overflow-x-auto pb-2">
          {rest.map((proof, index) => {
            const proofIsVideo = isVideo(proof.src);
            return (
              <motion.button
                key={proof.id}
                type="button"
                ...
                whileHover={{ y: -4 }}
                transition={springy}
                className="group relative w-40 shrink-0 overflow-hidden rounded-2xl border border-white/40 shadow-md sm:w-48"
              >
                {proofIsVideo ? (
                  <video
                    src={proof.src}
        ...
        {proofs.length > 1 && (
          <>
            <button
              ...
              className="absolute left-3 top-1/2 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-white/30 bg-black/20 text-white backdrop-blur-md transition hover:bg-white/30"
            >
              <LuChevronLeft size={20} />
            </button>
            <button
              ...
              className="absolute right-3 top-1/2 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-white/30 bg-black/20 text-white backdrop-blur-md transition hover:bg-white/30"
            >
...
function ProjectDetailCard({ project }: { project: Project }) {
  ...
  return (
    <motion.div
      layoutId={`project-${project.id}`}
      transition={springy}
      className="grid gap-6 overflow-hidden rounded-[3rem] border p-6 shadow-md backdrop-blur-lg md:grid-cols-[260px_1fr] md:p-8"
      style={{
